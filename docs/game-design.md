...
### Game Flow
1. **Deck Building**: Choose a Hero and construct 30-40 card decks within that Hero's color identity
...
#### Signature Cards
- Each Hero provides exactly one Signature Card tied to that Hero.
- Signature Cards are not part of your deck list and do not count toward deck size.
- Signature Cards may be played multiple times per game. After use, they move to a special cooldown/exile zone and become playable again after a defined cooldown in rounds. By default, cooldown is 2 rounds (tunable per Hero/card).
- Signature Cards follow normal targeting/timing rules of their printed type (e.g., Spell/Tactic) unless specified otherwise.
- Signature Cards cannot be generated by other effects unless a card explicitly allows generating Signature effects.
...
### Heroes
- Heroes are deck mascots chosen during deck construction.
- Each Hero defines a color identity that restricts which cards can be included in the deck.
- Each Hero provides a reusable Signature Card that can be played multiple times per game (see Signature Cards). Signature Cards return to a cooldown/exile zone after resolution and can be replayed after their cooldown.
- Heroes can provide passive deck modifications, such as starting resources, hand size adjustments, modifying starting structures, or global keywords for specific card tags.
- Heroes are not units on the board by default; they influence play through their passives and Signature Card.
...
#### Hero Rules
- You may have exactly one Hero per deck.
...
- Signature Card usage is limited by cooldown and any per-round limits the Hero defines. Unless stated otherwise, Signature Cards cannot be copied from graveyards and do not occupy a slot in your 30–40 card deck.
