			...
			for _, p := range plays {
				...
				if positionBlocked {
					...
					if cardRepo != nil {
						card, err := cardRepo.GetCard(nil, p.CardID)
						if err != nil {
							// Log error getting card
							evtLog.AddSimple(EventTypeEffect, "spawn_error", map[string]any{
								"playerIndex": playerIndex,
								"cardId":      p.CardID,
								"error":       err.Error(),
						...
						} else if card.UnitStats == nil {
							// Log unit stats are nil
							evtLog.AddSimple(EventTypeEffect, "spawn_error", map[string]any{
								"playerIndex": playerIndex,
								"cardId":      p.CardID,
								"reason":      "unit_stats_nil",
							})
						} else {
							// Successfully spawn the unit
